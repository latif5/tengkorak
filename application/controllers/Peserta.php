...
class Peserta extends Admin_Controller {
    ...
    $qry = "select p.id, p.nama, p.deskripsi, k.waktu, p.status, p.jumlah_undangan, p.jumlah_hadir from peserta p left join kehadiran k on p.id=k.id_peserta and (p.id LIKE ? or p.nama LIKE ? or p.deskripsi LIKE ? or k.waktu LIKE ? or p.status LIKE ?)  ORDER BY ".$sort_by." ".$sort_type." LIMIT ?, ?";
    $res = $this->db->query($qry, array("%".$sSearch."%","%".$sSearch."%","%".$sSearch."%","%".$sSearch."%","%".$sSearch."%",intval($start),intval($length)));

    $qry = "select count(*) as count from peserta p left join kehadiran k on p.id=k.id_peserta and (p.id LIKE ? or p.nama LIKE ? or p.deskripsi LIKE ? or k.waktu LIKE ? or p.status LIKE ?)";
